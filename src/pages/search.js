...
export const Search = () => {
    ...
    useEffect(() => {
        if(q) {
            if (template.filter(v => v.type === "title").length > 0)
                setTemplate(template.map(v => v?.type === "title" ? {name: q, type: "title"} : v))
            else
                setTemplate([...new Set(template), {name: q, type: "title"}])
        }
    }, [location])
    ...
    useEffect(() => {
        ...
        getData('sub-category').then(r => setList(r.data)).catch(e => setError(handleError(e)))
    },[])
    ...
    const nextPage = page => {
        ...
        else getData(`getFilterProduct?page=${page}`).then(r => {
            setProducts(r.data.data)
            setLoading(false)
        }).catch(err => {
            openNotificationWithIcon(handleError(err))
            setLoading(false)
        })
    }
        ...
        <Row gutter={[25, 20]} >
            <Col lg={4}>
                ...
                <Card>
                    ...
                    <Button style={{marginTop: '10px'}} icon={<SearchOutlined />} onClick={() =>getFilteredData("price", {type:"price",price: [parseInt(minValue), parseInt(maxValue)], name: "price "+parseInt(minValue)+' - '+parseInt(maxValue)})} />
                </Card>
            ...
            <Col xs={24} sm={12} md={18} lg={18} xl={18}>
                <Typography>You have searched: {q}</Typography>
                <Row gutter={[15, 5]}>
